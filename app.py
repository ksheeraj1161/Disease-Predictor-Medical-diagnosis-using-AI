...
def get_bot_response():
    ...
    if session['step'] == "BYE":
        ...
        if s.lower() == "yes":
            ...
            return "THANKS   " + name + ", hope this session was helpful for you"
...
def model_predict(img_path, model):
    ...
    print(preds)
    if (preds > 0):
        print("The x-ray is diagnosed with Pneumonia")
    else:
        print("The x-ray is Normal, No need to Worry")
    return preds
...
def upload():
    if request.method == 'POST':
        ...
        preds=int(preds1[0][0])
        os.remove(file_path)
    return ''

#======================model2============================================
Skin_model = 'models/skin_cancer.h5'

#Load your trained model
smodel = load_model(Skin_model)
#model._make_predict_function()  

def skin_predict(img_path, smodel):
    img =  load_img(img_path, target_size=(100, 125)) #target_size must agree with what the trained model expects!!
    # Preprocessing the image
    img = img_to_array(img)
    img = np.expand_dims(img, axis=0)
    preds = smodel.predict(img)
    print(preds)
    return preds

@app.route('/skin_upload', methods=['GET', 'POST'])
def skin_upload():
    if request.method == 'POST':
        # Get the file from post request
        f = request.files['file']
        ...
        f.save(file_path)
        # Make prediction
        preds1 = skin_predict(file_path, smodel)
...
with open('models/_model_.json', 'r') as f:
    model_json = f.read()
Breast_model = model_from_json(model_json)

# Load the model weights from a HDF5 file
Breast_model.load_weights('models/_model_.h5')

def Breast_predict(img_path, Breast_model):
    img =  load_img(img_path, target_size=(100,100)) #target_size must agree with what the trained model expects!!
    # Preprocessing the image
    img = img_to_array(img)
    img = np.expand_dims(img, axis=0)
    predictions_c = np.argmax(Breast_model.predict(img),axis=1)
    print(predictions_c)
    return predictions_c

@app.route('/breast_upload', methods=['GET', 'POST'])
def breast_upload():
    if request.method == 'POST':
        # Get the file from post request
        f = request.files['file']
        ...
        f.save(file_path)
        # Make prediction
        preds1 = Breast_predict(file_path, Breast_model)
        
        classes = {'TRAIN': ['Non Malignant (No Cancer)','Malignant'],'TEST': ['Non Malignant (No Cancer)','Malignant']}
        predicted_class = classes['TRAIN'][preds1[0]]
        print('We think that it is {}.'.format(predicted_class.lower()))
        os.remove(file_path)
    return '' 
...
def pneumoniaPage():
    return render_template('pneumonia.html',message='')

@app.route("/pneumoniaAbout", methods=['GET', 'POST'])
def pneumoniaAboutPage():
    return render_template('pneumoniaAbout2.html')

@app.route("/treatmentp", methods=['GET', 'POST'])
def pneumoniatPage():
    return render_template('treatmentp.html')
#======================skin============================================
@app.route("/skincancer", methods=['GET', 'POST'])
def skincancerPage():
    return render_template('skin.html')

@app.route("/skinabout", methods=['GET', 'POST'])
def skincanceraPage():
    return render_template('skinabout.html')

@app.route("/skincancert", methods=['GET', 'POST'])
def skincancertPage():
...
def breastcancerPage():
    return render_template('bc_predict.html')

@app.route("/breastabout", methods=['GET', 'POST'])
def breastcanceraPage():
    return render_template('breastabout.html')

@app.route("/treatmentf", methods=['GET', 'POST'])
def breastcancertPage():
    return render_template('treatmentf.html')
